...
for line in sys.stdin:
    ...
    if len(data) == 5:
        if data[0] != '"user_ptr_id"':    # skip header
            print "{0}\t{1}\t{2}\t{3}\t{4}\t{5}".format(data[0], 'A', data[1], data[2], data[3], data[4])
    elif len(data) == 19:
        if data[0] != '"id"':      # skip header
            print "{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}".format(data[3], 'B', data[0],\
            data[1], data[2], data[5], data[6], data[7], data[8], data[9])
